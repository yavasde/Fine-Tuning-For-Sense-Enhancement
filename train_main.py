...
def main():
    ...
    parser.add_argument("finetuning_method", type=str, help="Method of fine-tuning"
                        "(SCL, SPL or TASK)")
    parser.add_argument('lr', type=str, help='Learning Rate')
    parser.add_argument('epoch', type=str, help='Number of Epochs')
    parser.add_argument('-tau', type=str, help="Optional argument. Temperature of the loss."
                                                " Required with SCL and SPL.")
    ...
    print(f"Training Model for {model_type} with learning rate {learning_rate}")
    if args.tau:
        if model_type == "TASK":
            print("Tau is not used in task adaptation. Training will continue without tau.")
        else:
            tau = float(args.tau)
            print(f"Temp {tau}")
    else:
